...
class TERAModel:
    ...
    def fit(self, split: MD22Split) -> None:
        ...
        if self.train_steps > 0 or self.train_epochs > 0:
            if self._use_outer_graph_refresh():
                ...
                state = _build_training_state(split, init_lengthscale, self.m)
    ...
    def _train_likelihood_with_graph_refresh(
        ...
        while remaining_epochs > 0:
            ...
            state = _build_training_state(split, current_lengthscale, self.m)
    ...
    def _train_likelihood(
        ...
        *,
        ...
        for step in range(1, total_steps + 1):
            ...
            lengthscale = self._apply_lengthscale_bounds(lengthscale)
            ...
            if self.log_every > 0 and (step == 1 or step % self.log_every == 0 or step == total_steps):
                cur_lengthscale = (torch.exp(log_lengthscale) if self.learn_lengthscale else init_lengthscale.detach())
        ...
        with torch.no_grad():
            final_lengthscale = torch.exp(log_lengthscale) if self.learn_lengthscale else init_lengthscale
...
def _build_training_state(split: MD22Split, init_lengthscale: torch.Tensor, m: int) -> _TERATrainingState:
    ...
    return _TERATrainingState(X=X, y=y, g=g, cond_sets=cond_sets, sample_positions=sample_positions)
...
def _solve_psd(A: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    """Solve a possibly singular symmetric PSD system in TERA factors.

    The reduced-gradient covariance is a Schur complement covariance for the
    projected gradient statistics.  With exact gradients, and especially with
    the stacked m^2 representation used here, this covariance is only guaranteed
    to be positive semidefinite.  Small negative eigenvalues can also be created
    by float32 cancellation in the direct reduced-block formulas.  A strict
    Cholesky factorization is therefore too strong for this block.

    We first use the fast Cholesky path for well-conditioned factors.  If that
    fails, we fall back to the Moore-Penrose solve, which is the appropriate
    Gaussian-conditioning operation for degenerate conditioning statistics.
    """
    A = 0.5 * (A + A.T)
    try:
