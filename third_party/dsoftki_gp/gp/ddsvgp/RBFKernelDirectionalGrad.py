# Adapated from: https://github.com/mishapadidar/GP-Derivatives-Variational-Inference/tree/main
...
class RBFKernelDirectionalGrad(RBFKernel):
    ...
    def forward(self, x1, x2, diag=False, **params):
        ...
        if not diag:
            ...
            # print(outer[:,pi1].shape, self.lengthscale.shape)
            # l = self.lengthscale.repeat(outer[:,pi1].shape[0], outer[:,pi1].shape[0]).unsqueeze(-2)
            outer1 = outer[:,pi1] / self.lengthscale.unsqueeze(-2)
            # outer1 = outer[:,pi1] / l
            # print(outer[:,pi1].shape, self.lengthscale.shape, outer1.shape)
...
if __name__ == '__main__':
    ...
    k = RBFKernelDirectionalGrad(ard_num_dims=None)
    params = {'v1':v1,'v2':v2}
    ...
    L = psd_safe_cholesky(delazify(K).double(), jitter=settings.cholesky_jitter.value(torch.float64))
    TriangularLazyTensor(L)
